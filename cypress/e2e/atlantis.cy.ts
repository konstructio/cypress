...
let git_owner: string='',git_token: string='',git_username: string='',subdomain: string='',domain: string='';
...
const checkStatusCode = () => {
    ...
    return (
        cy.exec(`node git-status.js ${git_username} ${git_token} ${git_owner}`, { timeout: 1200000 }).then((result)=>{
            try {
...
describe("Altantis Check",()=>{
    ...
    it("Run Atlanis Shell",()=>{
        getgitceredintals().then(({git_owner,git_token,subdomain,domain})=>{ 
            if(!checkStatusCode()) throw Error ("Error in running applying changes to gitops repo")
        })
    })
